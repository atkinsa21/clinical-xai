...
class OnnxModel:
    ...
    def __init__(self, model_path: str | Path, feature_names: list[str] | None = None) -> None:
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        raw = self._session.run([self._labels_output_name], {self._input_name: X_arr})[0]
        return raw.ravel()
    
    def predict_proba(self, X: pd.DataFrame) -> np.ndarray:
        self._check_columns(X)
        X_arr = X.to_numpy(dtype=np.float32)
        return self._session.run([self._proba_output_name], {self._input_name: X_arr})[0]
