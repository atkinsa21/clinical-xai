...
def test_feature_names_none_does_not_raise(onnx_model):
    model, fx = onnx_model
...
def test_predict_returns_1d_array_of_length_n(onnx_model):
    model, fx = onnx_model
...
def test_predict_proba_returns_2d_array(onnx_model):
    model, fx = onnx_model
    probs = model.predict_proba(fx.X_test)
    assert probs.shape == (len(fx.X_test), 2)


def test_predict_proba_rows_sum_to_one(onnx_model):
    model, fx = onnx_model
    probs = model.predict_proba(fx.X_test)
    assert probs.sum(axis=1) == pytest.approx(1.0, abs=1e-5)


def test_predict_proba_values_in_zero_one(onnx_model):
    model, fx = onnx_model
    probs = model.predict_proba(fx.X_test)
    assert (probs >= 0).all() and (probs <= 1).all()


def test_predict_raises_on_wrong_feature_names(onnx_model):
    _, fx = onnx_model
...
def test_predict_proba_raises_on_wrong_feature_names(onnx_model):
    _, fx = onnx_model
    model = OnnxModel(fx.model_path, feature_names=list(fx.X_train.columns))
    reversed_columns = fx.X_test[list(reversed(fx.X_test.columns))]
    with pytest.raises(ValueError):
...
def test_predict_runs_when_feature_names_match(onnx_model):
    model, fx = onnx_model
    preds = model.predict(fx.X_test)
    probs = model.predict_proba(fx.X_test)
    assert preds.shape == (len(fx.X_test),)
